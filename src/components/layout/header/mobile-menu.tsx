...
export default function MobileMenu() {
	...
	let menuList = data.map((columnData:any) => 
		columnData.columns.map((columnItemData:any) =>
			columnItemData.columnItems.map((columnItemsItemData:any) => {
				let menuData = cloneDeep(columnItemsItemData);
				const { columnItemItems, ...remainingMenu } = menuData;
				menuData = {
					...remainingMenu,
					...(columnItemItems.length > 0 && { subMenu: columnItemItems })
				}
				return menuData
			})
		)
	...
	menuList = [ ...menuList, ...additionalMenu ]
		...
		data.label && (
			<li className={`mb-0.5 ${className}`}>
				<div className="flex items-center justify-between">
					<div
						onClick={() => {
							if(!data.subMenu && data.path){
								Router.push(data.path)
							}
							else if (hasSubMenu) {
								Router.push(`/categories/${data.label}-${data.id}`);
							} else {
								Router.push(`/category/${data.unique_id}`);
							}
						}}
