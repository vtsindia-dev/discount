...
const Widgets: React.FC<WidgetsProps> = ({ widgets }) => {
	return (
		<Container>
			...
			<div className="flex flex-wrap justify-center sm:justify-start md:justify-center md:flex-nowrap gap-5 md:gap-9 xl:gap-5  pb-9 md:pb-14 lg:pb-16 2xl:pb-20 3xl:pb-24 lg:mb-0.5 2xl:mb-0 3xl:-mb-1">
				{widgets?.map((widget) => (
					<WidgetLink
						key={`footer-widget--key${widget.id}`}
						data={widget}
						className="pb-3 md:pb-0 w-64"
					/>
				))}
			</div>
		</Container>
