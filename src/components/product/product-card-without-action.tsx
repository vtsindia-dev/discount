...
const ProductCardWithoutAction: FC<ProductProps> = ({
	product,
	className = "",
	contactClassName = "",
	...
	const { isAuthorized, accountDetails, setModalView, openModal } = useUI();
	...
	function handleSuccessCbk(data: any) {
		const { isWishlisted, message } = data;
		const props:any = {
			type: "dark",
		...
		if (typeof isWishlisted === undefined || typeof isWishlisted === null) {
			toast("Something went wrong please try again later", props);
		} else {
			toast(message, props);
		}
			...
			>
				...
				>
					<Image
						...
						className={cn("bg-white object-cover rounded-s-md h-auto", {
...
export default ProductCardWithoutAction;
