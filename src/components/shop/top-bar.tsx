...
const SearchTopBar = () => {
	const { openFilter, displayFilter, closeFilter, productList } = useUI();
	const { t } = useTranslation("common");
	const { locale } = useRouter();
	const dir = getDirection(locale);
	const contentWrapperCSS = dir === "ltr" ? { left: 0 } : { right: 0 };
	const { pageName, totalProduct } = productList;
	return (
		<div className="block lg:flex justify-between items-center mb-7">
			<div className="flex items-center mb-2 lg:mb-0 ">
				<Text variant="pageHeading" className="inline-flex pb-1 capitalize">
					{pageName}
				</Text>
				<div className="flex-shrink-0 text-body text-xs md:text-sm leading-4 pe-4 md:me-6 ps-2 block lg:hidden">
					({totalProduct} {t("text-items")})
				</div>
			</div>
				...
				<div className="flex items-center justify-end">
					...
					<ListBox
						options={[
							...
							{ name: "text-price-low-high", value: "price-low-high" },
							{ name: "text-price-high-low", value: "price-high-low" },
						]}
					/>
				</div>
			</div>
