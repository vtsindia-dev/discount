...
import { useHeroBannerQuery } from "@framework/home/get-hero-banner";
...
const HeroWithCategory: React.FC<Props> = ({
	...
	// 	limit: 10,
	// });
	const { data: heroBanner } = useHeroBannerQuery();
	return (
		<div
		...
		>
			...
			<div className="heightFull col-span-full row-span-full">
				<Carousel
					pagination={{
						clickable: true,
					}}
				...
				>
					{heroBanner?.data?.map((banner: any) => (
						<SwiperSlide key={`banner--key${banner.id}`}>
							<BannerCard banner={banner} className="xl:h-full" classNameInner="hero-banner" />
						</SwiperSlide>
					))}
				</Carousel>
			</div>
